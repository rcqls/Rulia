...
jlvalue_eval_rexpr <- function(rexpr, parent_envir = parent.frame()) {
  jlval <- if (class(rexpr) == "name") {
    ...
    #  jlvalue_eval(obj)
    #} else 
    if (is_variable(obj, parent_envir)) {
...
jlvalue_eval_rexprs <- function(rexprs, parent_envir) {
  # rexpr is generally the result of substitute(obj)
  rexprs <- as.list(rexprs)[-1]
  nms <- names(rexprs)
  res <- lapply(seq_along(rexprs),
    function(i) jlvalue_eval_rexpr(rexprs[[i]], parent_envir)
  )
  names(res) <- nms
  res
}
...
# is_variable <- function(name, envir) {
...
is_variable <- function(name, envir) {
  ...
  !is.null(r) &&
    (regexpr("^[a-z,A-Z,.][a-z,A-Z,.,0-9]*", name) > 0) &&
    !is.function(r)
}
