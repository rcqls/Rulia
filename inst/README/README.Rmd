...
After installing `RCall.jl` and loading `jlinclude(Rulia::RCallPtr)`, one can have access to this feature.
...
This feature as illustrated below can be applied to `R` vector of type `double`, `integer`, `double` but not `character`.
